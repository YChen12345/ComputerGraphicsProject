...
struct AnimationSequence // This holds rescaled times
{
	...
	void calcFrame(float t, int& frame, float& interpolationFact)
	{
		...
		frame = MMin(frame, (int)(frames.size() - 1));
	}
	...
	{
		return MMin(frame + 1, (int)(frames.size() - 1));
	}
	Matrix interpolateBoneToGlobal(Matrix* matrices, int baseFrame, float interpolationFact, Skeleton* skeleton, int boneIndex)
	{
		Matrix scale = Matrix::CreateScaling(interpolate(frames[baseFrame].scales[boneIndex], frames[nextFrame(baseFrame)].scales[boneIndex], interpolationFact));
		...
		Matrix translation = Matrix::CreateTranslation(interpolate(frames[baseFrame].positions[boneIndex], frames[nextFrame(baseFrame)].positions[boneIndex], interpolationFact));

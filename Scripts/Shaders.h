...
public:
	...
	void loadPS(Core* core, std::string hlsl)
	{
		...
		if (FAILED(hr))
		{
			std::string err = status ? (char*)status->GetBufferPointer() : "Unknown PS compile error";
			OutputDebugStringA(err.c_str());
			MessageBoxA(nullptr, err.c_str(), "Pixel Shader Compile Error", MB_OK);
			exit(0);
		}
	...
	void loadVS(Core* core, std::string hlsl)
	{
		...
		if (FAILED(hr))
		{
			std::string err = status ? (char*)status->GetBufferPointer() : "Unknown VS compile error";
			OutputDebugStringA(err.c_str());
			MessageBoxA(nullptr, err.c_str(), "Vertex Shader Compile Error", MB_OK);
			exit(0);
		}
...
public:
	...
	std::string readFile(std::string filename)
	{
		...
		if (!file.is_open())
		{
			std::string msg = "Failed to open shader file: " + filename + "\n"
				"Tip: copy this .txt to the exe output folder (x64/Debug or x64/Release).";
	...
	void updateTexturePS(Core* core, std::string shaderName, std::string textureName, int heapOffset)
	{
		shaders[shaderName].updateTexturePS(core, textureName, heapOffset);
	}

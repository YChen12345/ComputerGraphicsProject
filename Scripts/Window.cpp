...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		if (window->keys[(unsigned int)wParam] < 2) {
			window->keys[(unsigned int)wParam] += 1;
		}
		else
		{
			window->keys[(unsigned int)wParam] = 2;
		}
		return 0;
	}
	case WM_KEYUP:
	{
		window->keys[(unsigned int)wParam] = 3;
		return 0;
	}
	case WM_LBUTTONDOWN:
	{
		window->updateMouse(WINDOW_GET_X_LPARAM(lParam), WINDOW_GET_Y_LPARAM(lParam));
		if (window->mouseButtons[0] < 2) {
			window->mouseButtons[0] += 1;
		}
		else
		{
			window->mouseButtons[0] = 2;
		}
		return 0;
	}
	case WM_LBUTTONUP:
	{
		window->updateMouse(WINDOW_GET_X_LPARAM(lParam), WINDOW_GET_Y_LPARAM(lParam));
		window->mouseButtons[0] = 3;
		return 0;
	}
	case WM_RBUTTONDOWN:
	{
		window->updateMouse(WINDOW_GET_X_LPARAM(lParam), WINDOW_GET_Y_LPARAM(lParam));
		if (window->mouseButtons[2] < 2) {
			window->mouseButtons[2] += 1;
		}
		else
		{
			window->mouseButtons[2] = 2;
		}
		return 0;
	}
	case WM_RBUTTONUP:
	{
		window->updateMouse(WINDOW_GET_X_LPARAM(lParam), WINDOW_GET_Y_LPARAM(lParam));
		window->mouseButtons[2] = 3;
		return 0;
	}
	case WM_MBUTTONDOWN:
	{
		window->updateMouse(WINDOW_GET_X_LPARAM(lParam), WINDOW_GET_Y_LPARAM(lParam));
		if (window->mouseButtons[1] < 2) {
			window->mouseButtons[1] += 1;
		}
		else
		{
			window->mouseButtons[1] = 2;
		}
		return 0;
	}
	case WM_MBUTTONUP:
